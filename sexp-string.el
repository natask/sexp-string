  ...
  (let* ((predicates (intern-soft (concat library-name "-predicates")))
         ...
         (pexs `((query sum (opt eol))
                 ...
                 (positive-term (or
                                    ...
                                    (and strict-plain-string `(s -- (cons ',,default s)))
                                    (and expansive-plain-string `(s -- (cons ',,default s)))))
                 ...
                 (strict-plain-string (list (+ (or unquoted-arg quoted-arg (substring (or separator negation ))))))
                 (expansive-plain-string (list (+ (or unquoted-arg quoted-arg (substring (or separator negation ))))))
                 (negation "!")
                 (separator "," )
                 (operator (or "`and" "`or"))
                 (brackets (or "(" ")"))
